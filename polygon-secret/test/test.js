...
describe('PolygonToSecret Test', function () {
  let PolygonToSecretFactory, AxelarMockFactory, polygonToSecret, axelarMock
  let owner, user1
  ...
  before(async function () {
    ...
    PolygonToSecretFactory = await ethers.getContractFactory('PolygonToSecret')
  ...
  beforeEach(async function () {
    ...
    polygonToSecret = await PolygonToSecretFactory.deploy(
      axelarMock.address,
      axelarMock.address,
      chainName,
    )
  ...
  it('Send payload', async function () {
    ...
    await polygonToSecret
      .connect(user1)
      .send('destinationChain', 'destinationAddress', message, { value: fee })
    ...
    // argValues
    let [chain, sourceAddress, msgPayload] = ethers.utils.defaultAbiCoder.decode(
      ['string', 'string', 'bytes'],
      argValues,
    )
    expect(chain).to.equal(chainName)
    expect(ethers.utils.getAddress(sourceAddress)).to.equal(polygonToSecret.address)

    // executeMsgPayload
    let [sender, decoded_message] = ethers.utils.defaultAbiCoder.decode(
      ['string', 'string'],
      msgPayload,
    )
  ...
  it('Receive payload', async function () {
    ...
    let returnPayload = ethers.utils.defaultAbiCoder.encode(
      ['string', 'string'],
      [payload_sender, payload_message],
    )

    // call _execute()
    await axelarMock.executeFromGateway(
      polygonToSecret.address,
      'source_chain',
      'source_address',
      returnPayload,
    )
    ...
    let storedMessage = await polygonToSecret.storedMessage()
    expect(storedMessage.sender).to.equal(payload_sender)
    expect(storedMessage.message).to.equal(payload_message)
  })

  it('Decode Utility', async function () {
    // Utility test to decode the payload listed by axelarScan
    // Replace {gmpPayload} with the hex string to be decoded
    let gmpPayload =
      ''
